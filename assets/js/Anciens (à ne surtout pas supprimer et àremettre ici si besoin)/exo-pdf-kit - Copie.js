...
(function(){
  ...
  function exerciseHTML(def, st){
    ...
    // Fallback : on rend “hors écran” puis on lit .equ
    if (typeof def.render === 'function') {
      try {
        ...
        def.render(tmp, st); // la page met l’énoncé dans .equ
...
function solutionHTML(def, st){
  ...
  // 🔁 Fallback générique : on rend l'exo offscreen, on appelle def.solution(), puis on lit #res
  if (typeof def.render === 'function' && (typeof def.solution === 'function' || typeof def.correct === 'function')) {
    try {
      ...
      // 1) construire la structure (avec .equ et #res)
      def.render(tmp, st);

      // 2) demander la solution si dispo, sinon à défaut lancer correct() (souvent ça remplit aussi #res)
      ...
      // 3) récupérer ce que la page affiche dans la zone résultat
      const res = tmp.querySelector('#res');
      const html = res && res.innerHTML.trim() ? res.innerHTML : tmp.innerHTML;

      tmp.remove();
      return html && html.trim() ? html : '<div class="steps"><div class="step">Corrigé non disponible pour ce type.</div></div>';
    } catch (e) {
