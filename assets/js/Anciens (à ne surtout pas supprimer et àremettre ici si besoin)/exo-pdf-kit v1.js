...
function solutionHTML(def, st){
  ...
  // Fallback générique : rendu offscreen + solution() + conversion en statique
  if (typeof def.render === 'function' && (typeof def.solution === 'function' || typeof def.correct === 'function')) {
    try {
      ...
      else def.correct(tmp, st);

      // on retire le champ de réponse si présent pour ne pas polluer le rendu
      tmp.querySelectorAll('input#reponse, input[name=reponse]').forEach(el=>el.remove());
      // pas de conversion globale : on veut le même bloc que l'UI
      // staticizeForPrint(tmp); // inutile ici
      ...
      return html && html.trim()
        ? html
        : '<div class="steps"><div class="step">Corrigé non disponible pour ce type.</div></div>';
    } catch (e) {
