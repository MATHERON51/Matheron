...
/* ==== Patch anti-collage pour expressions math en gras ====
   Insère une fine espace insécable (U+202F) avant/après <b>…</b>, <strong>…</strong>,
   et éléments math dédiés (.equ, .math, code.math) quand ils sont collés à des mots.
   Idempotent et appliqué aussi aux contenus injectés dynamiquement.
*/
(function(){
  var NBSP_NARROW = '\u202F'; // fine espace insécable FR
  function isWordEnd(s){ return /[\p{L}\p{N}\)]$/u.test(s||''); }     // lettre/chiffre ou ')'
  function isWordStart(s){ return /^[\p{L}\p{N}\(]/u.test(s||''); }   // lettre/chiffre ou '('
  ...
  function ensureSpaceBefore(el){
    ...
    if(prev.nodeType === Node.TEXT_NODE){
      var t = prev.data;
      if(t && isWordEnd(t) && !/[ \u00A0\u202F]$/.test(t)){
        prev.data = t + NBSP_NARROW;
      }
    }else if(prev.nodeType === Node.ELEMENT_NODE && prev.tagName !== 'BR'){
      el.parentNode.insertBefore(document.createTextNode(NBSP_NARROW), el);
    }
  }

  function ensureSpaceAfter(el){
    var next = el.nextSibling;
    if(!next) return;
    if(next.nodeType === Node.TEXT_NODE){
      var t = next.data;
      if(t && isWordStart(t) && !/^[ \u00A0\u202F]/.test(t)){
        next.data = NBSP_NARROW + t;
      }
    }else if(next.nodeType === Node.ELEMENT_NODE && next.tagName !== 'BR'){
      el.parentNode.insertBefore(document.createTextNode(NBSP_NARROW), next);
    }
  }

  function fixTightInline(root){
    root = root || document;
    try{
      var nodes = root.querySelectorAll('b,strong,span.equ,span.math,code.math');
      nodes.forEach(function(el){ ensureSpaceBefore(el); ensureSpaceAfter(el); });
    }catch(e){ /* noop */ }
  }

  // Expose dans DevRules si présent, sinon global
  window.DevRules = window.DevRules || {};
  window.DevRules.fixTightInline = fixTightInline;

  // Lancer au chargement
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', function(){ fixTightInline(document); });
  }else{
    fixTightInline(document);
  }

  // Lancer aussi sur contenus ajoutés dynamiquement (nouvel énoncé, solution, etc.)
  try{
  ...
  }catch(e){ /* MutationObserver indisponible */ }

  // Intégration douce avec un éventuel attacheur existant
  if(typeof window.DevRules.attachCleaner === 'function'){
    var prev = window.DevRules.attachCleaner;
    window.DevRules.attachCleaner = function(root){
      try{ prev.call(window.DevRules, root); }catch(e){}
      try{ fixTightInline(root || document); }catch(e){}
    };
  }
})();
