/*! VB mode prelude: inputs-only for DevRules cleaner
   - Set window.DEVRULES_MODE='inputs-only' BEFORE including this file to scope cleaning to inputs only.
   - This prelude disables the DOM cleaner and re-enables normalization only on inputs/textareas.
*/
;(function(g){
  try{
    if(!g) return;
    g.DevRules = g.DevRules || {};
    var MODE = g.DEVRULES_MODE || 'all';
    if (MODE === 'inputs-only'){
      // Prevent the base file from attaching the global DOM cleaner
      g.__DEV_RULES_DEDUP_CLEANER__ = true;

      // Input-only cleaner: runs on DOMContentLoaded
      var SEL = 'input[type="text"], textarea, .eqline input, .eqline textarea, input.math, textarea.math, .math-input, .kbd-target';
      function normalizeValue(el){
        try{
          if (!el || typeof el.value !== 'string') return;
          var cleanString = (g.DevRules && g.DevRules.cleanString) ? g.DevRules.cleanString : null;
          if (!cleanString) return;
          var v = el.value, nv = cleanString(v);
          if (nv !== v){
      ...
      function boot(){
        try{
          if (g.document && g.document.querySelectorAll){
            var list = g.document.querySelectorAll(SEL);
            for (var i=0;i<list.length;i++) normalizeValue(list[i]);
          }
          g.document && g.document.addEventListener('input', handler, true);
          g.document && g.document.addEventListener('change', handler, true);
          g.document && g.document.addEventListener('keyup', handler, true);
                ...
                if (m.addedNodes){
                  for (var j=0;j<m.addedNodes.length;j++){
                    ...
                    if (n && n.nodeType === 1 && n.querySelectorAll){
                      var list2 = n.querySelectorAll(SEL);
                      for (var k=0;k<list2.length;k++) normalizeValue(list2[k]);
                    }
                  }
                }
              }
            });
            mo.observe(g.document.documentElement, {subtree:true, childList:true});
          }
        }catch(_){}
      }
      if (g.document){
