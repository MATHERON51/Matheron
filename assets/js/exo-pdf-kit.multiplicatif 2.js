...
(function(){
  ...
  // 1) Si l’auteur fournit textHTML/text, on respecte
  if (typeof def.textHTML === 'function') return def.textHTML(st);
  if (typeof def.text === 'function')     return def.text(st);

  // 2) Rendu offscreen MAIS avec une taille non nulle (clé !) pour que le SVG se pose
  if (typeof def.render === 'function') {
    try {
      ...
      // Purge couche “user” (clics, constructions temporaires)
      const userLayer = tmp.querySelector('g[data-layer="user"]');
      if (userLayer) userLayer.innerHTML = '';

      // Nettoyage rapide de l’énoncé (droite)
      const equ = tmp.querySelector('.equ') || tmp.querySelector('.hint') || null;
      const right = equ ? equ.cloneNode(true) : tmp.cloneNode(true);
      // Inputs/textarea → traits ; UI à virer
      right.querySelectorAll('input, textarea, button, select, option, .tick, .optionline, [data-math-kbd], .kbd-host')
        .forEach(n=>n.remove());
      right.querySelectorAll('[contenteditable]').forEach(n=>n.removeAttribute('contenteditable'));

      // Export de la courbe (gauche) — priorité à une fonction globale si fournie côté page
      function __svgToDataImg(root, opts){
        ...
        const widthPx  = (opts && opts.widthPx) || 360;
        const blockW   = (opts && opts.blockWidth) || '100%';
        try{
          const xml  = new XMLSerializer().serializeToString(c);
          const data = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(xml);
          return `<img src="${data}" style="display:block;width:${blockW};max-width:${widthPx}px;height:auto">`;
        }catch(e){
          // Fallback inline <svg>
          c.setAttribute('width',  String(widthPx));
          c.setAttribute('height', String(widthPx));
          c.style.display='block'; c.style.width='100%'; c.style.height='auto';
          return c.outerHTML;
        }
      }
      ...
      if (typeof window.cloneCurveForPDF_img === 'function'){
        svgHTML = window.cloneCurveForPDF_img(tmp, { widthPx: 360, blockWidth:'100%' });
      } else if (typeof window.cloneCurveForPDF === 'function'){
        svgHTML = window.cloneCurveForPDF(tmp, { widthPx: 360, blockWidth:'100%' });
      } else {
        svgHTML = __svgToDataImg(tmp, { widthPx: 360, blockWidth:'100%' });
      }

      // Mise en page de base : si on a les deux, on tente 2 colonnes ; sinon empilement
      const statement = (svgHTML && right && right.innerHTML.trim())
        ? (`<table style="width:100%;border-collapse:collapse;table-layout:fixed">
              <tr>
                <td style="width:54%;vertical-align:top;padding:0 12px 0 0">${svgHTML}</td>
                <td style="width:46%;vertical-align:top;padding:0">${right.innerHTML}</td>
              </tr>
            </table>`)
        : (`<div>${svgHTML||''}</div><div style="margin-top:6px">${right?right.innerHTML:''}</div>`);
