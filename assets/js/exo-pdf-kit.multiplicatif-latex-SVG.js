...
if (typeof opts.beforeRender === 'function') {
  try {
    const out = await opts.beforeRender(def, st, /* withSolutions? */ false);
    ...
    console.warn('[exo-pdf-kit] beforeRender(statement) a échoué:', e);
  }
}
if (!enonceHTML) {
  enonceHTML = exerciseHTML(def, st);
}
...
if (typeof opts.beforeRender === 'function') {
  try {
    const out = await opts.beforeRender(def, st, /* withSolutions? */ true);
    if (typeof out === 'string') {
      corrigeHTML = out;
    } else if (out && typeof out.solution === 'string') {
      corrigeHTML = out.solution;
    }
  } catch(e){
    console.warn('[exo-pdf-kit] beforeRender(solution) a échoué:', e);
  }
}
if (!corrigeHTML) {
  corrigeHTML = solutionHTML(def, st);
}
