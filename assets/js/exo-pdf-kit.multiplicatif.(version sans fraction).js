  ...
  function buildControlsUI(opts){
    ...
    wrap.innerHTML = `
      ...
      <div class="row-inline" style="display:flex;flex-wrap:wrap;gap:10px;align-items:center">
        ...
        <button id=\"btn-word\" class=\"btn\">🗎 Word (.doc)</button>
      </div>
      <div class="row-inline" style="display:flex;flex-wrap:wrap;gap:10px;align-items:center">
...
<body>
  ...
  // --- Export Word (.doc) — mode print-like (~300 DPI) ---
  async function openWord(nb, mix, withSolutions, header, opts){
    const html = buildPrintableHTML(nb, mix, withSolutions, header, opts);
    const htmlDoc = await convertSvgsForWord(html); // rasterisation HQ + largeur 170mm
    const blob = new Blob(['\ufeff', htmlDoc], {type:'application/msword'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = (opts.title || 'Fiche_MatHeron') + '.doc';
    document.body.appendChild(a); a.click();
    setTimeout(()=>{ URL.revokeObjectURL(url); a.remove(); }, 100);
  }

  // Convertit tous les <svg> en <img> PNG (≈300 DPI) pour compatibilité Word
  
async function convertSvgsForWord(html){
  // Word export with normal margins; clamp images to <= 158mm (fits 2.54 cm margins)
  const CONTENT_MM = 158;                      // safe content width (mm)
  const DPI = 300;                             // raster DPI
  const PX_PER_MM = DPI / 25.4;
  const TARGET_PX = Math.max(1400, Math.round(CONTENT_MM * PX_PER_MM)); // ≈ 1866 px
  ...
  try{
    ...
    await Promise.all(svgs.map(async (svg)=>{
      // Serialize and slightly increase strokes
      let xml = new XMLSerializer().serializeToString(svg);
      xml = xml.replace(/stroke-width\s*=\s*\"([0-9]*\.?[0-9]+)\"/g, (m,v)=>`stroke-width="${(parseFloat(v)*1.3).toFixed(2)}"`);
      xml = xml.replace(/stroke-width\s*:\s*([0-9]*\.?[0-9]+)/g, (m,v)=>`stroke-width:${(parseFloat(v)*1.3).toFixed(2)}`);

      // Determine aspect ratio from viewBox
      let ratio = 0.12;
      const m = xml.match(/viewBox\s*=\s*\"[^"]*?\s+[^"]*?\s+([0-9.]+)\s+([0-9.]+)\"/);
      if(m){
        const wv = parseFloat(m[1]||'980'), hv = parseFloat(m[2]||'110');
        if(wv>0 && hv>0) ratio = hv/wv;
      }
      const Wpx = TARGET_PX;
      const Hpx = Math.max(120, Math.round(Wpx * ratio));

      // Force explicit width/height on the root <svg> before rasterizing
      try{
        const sdoc = new DOMParser().parseFromString(xml, 'image/svg+xml');
        const root = sdoc.documentElement;
        root.setAttribute('width', String(Wpx));
        root.setAttribute('height', String(Hpx));
        xml = new XMLSerializer().serializeToString(sdoc);
      }catch(_){}

      const png = await svgStringToPng(xml, Wpx, Hpx);
      const img = doc.createElement('img');
      img.setAttribute('src', png);
      img.setAttribute('style', 'width:158.0mm;height:auto;display:block');
        img.setAttribute('width', '597');
      svg.parentNode && svg.parentNode.replaceChild(img, svg);
    }));

    // Global clamp to keep images inside page margins
    const style = doc.createElement('style');
    style.textContent = `.sol-graph img,.sol-steps img,.graph img{display:block;height:auto}`;
  ...
  function svgStringToPng(svgText, width, height){
    return new Promise((resolve)=>{
      const img = new Image();
      const svgUrl = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svgText);
      img.onload = () => {
        const canvas = document.createElement('canvas');
        canvas.width = Math.max(1, Math.round(width||800));
        canvas.height = Math.max(1, Math.round(height||200));
        const ctx = canvas.getContext('2d');
        ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
        try{
          const png = canvas.toDataURL('image/png');
          resolve(png);
        }catch(e){ resolve(svgUrl); }
      };
  ...
  // Bouton Word (.doc) — rasterisation HQ
  ui.querySelector('#btn-word').addEventListener('click', async function(){
    const n = Math.max(1, Math.min(opts.max, parseInt((ui.querySelector('#pdf-count')||{}).value || '10', 10)));
    const mix = !!(ui.querySelector('#pdf-mix')||{}).checked;
    const withSol = !!(ui.querySelector('#pdf-sol')||{}).checked;
    const header = {
    ...
    await openWord(n, mix, withSol, header, opts);
  });
}
