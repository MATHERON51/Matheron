...
// global snapshot store (per def.id) to keep SAME random state for statement & solution
  window.__PDF_EXO_SNAPSHOT = window.__PDF_EXO_SNAPSHOT || {};

  // Prefer page-provided text HTML
  if (typeof def.textHTML === 'function') return def.textHTML(st);
  if (typeof def.text === 'function')     return def.text(st);

  // Fallback: render offscreen with non-zero size
  if (typeof def.render === 'function') {
    ...
    try{
      // Use cached params if page saved them
      const saved = (window.__EXO_PARAMS_CACHE && window.__EXO_PARAMS_CACHE[def.id]) || st || {};
      def.render(tmp, saved);

      // Remember full state (including random order) for the solution call
      try{
        const snap = tmp._state || (tmp.dataset && tmp.dataset.state ? JSON.parse(tmp.dataset.state).params : null);
        if (snap) window.__PDF_EXO_SNAPSHOT[def.id] = snap;
      }catch(e){ /* ignore */ }

      // Strip interactive UI in a clone of .equ
      const equ = tmp.querySelector('.equ') || tmp;
      const right = equ.cloneNode(true);
      const strip = (root)=>{
        const $$ = (sel)=>Array.from(root.querySelectorAll(sel));
        $$('input, textarea, button, select, option, .tick, .optionline, [data-math-kbd], .kbd-host').forEach(n=>n.remove());
        $$('[contenteditable]').forEach(n=>n.removeAttribute('contenteditable'));
        // underline for inputs already removed above (handled by page if needed)
        return root;
      };
      strip(right);
      ...
      let leftHTML = '';
      try{
        if (typeof window.cloneCurveForPDF_img === 'function') leftHTML = window.cloneCurveForPDF_img(tmp, {widthPx:360, blockWidth:'100%'});
        else if (typeof window.cloneCurveForPDF === 'function') leftHTML = window.cloneCurveForPDF(tmp, {widthPx:360, blockWidth:'100%'});
        else leftHTML = '';
      }catch(e){ leftHTML=''; }
      ...
      console.warn('[exo-pdf-kit] exerciseHTML fallback error', e);
...
// Use the EXACT same random state captured in exerciseHTML
  window.__PDF_EXO_SNAPSHOT = window.__PDF_EXO_SNAPSHOT || {};
  ...
  if (typeof def.render === 'function' && (typeof def.solution === 'function' || typeof def.correct === 'function')) {
    ...
    document.body.appendChild(tmp);
    try{
      // Build state with snapshot if we have one, else regular saved
      const snapshot = window.__PDF_EXO_SNAPSHOT[def.id];
      const saved = snapshot ? {params: snapshot} : ((window.__EXO_PARAMS_CACHE && window.__EXO_PARAMS_CACHE[def.id]) || st || {});

      def.render(tmp, saved);

      // If we had a snapshot, enforce it on host._state (some renderers reshuffle order)
      if (snapshot) tmp._state = snapshot;

      // Now build the solution on that same state
      if (typeof def.solution === 'function') {
        try { def.solution(tmp, saved); } catch(e){ /* ignore */ }
      } else if (typeof def.correct === 'function') {
        try { def.correct(tmp, saved); } catch(e){ /* ignore */ }
      }

      // We only return the .steps/.solution block (not the statement again)
      const strip = (root)=>{
        const $$ = (sel)=>Array.from(root.querySelectorAll(sel));
        $$('input, textarea, button, select, option, .tick, .optionline, [data-math-kbd], .kbd-host').forEach(n=>n.remove());
        $$('[contenteditable]').forEach(n=>n.removeAttribute('contenteditable'));
      ...
      const html  = steps.outerHTML || steps.innerHTML || '';
      return html && html.trim() ? html : 'Corrigé indisponible';
    }catch(e){
      console.warn('[exo-pdf-kit] solutionHTML fallback error', e);
