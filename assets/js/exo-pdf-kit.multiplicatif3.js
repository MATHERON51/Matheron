...
// Hook-friendly: if page provided beforeRender returns string/object, ExoPDF will use it.
  // This fallback renders offscreen (BUT at non-zero size) and assembles a 2-col layout if an SVG exists.
  // It also strips interactive UI (inputs, buttons, ticks...) from the statement.
  // Returns a STRING of HTML.
  // NOTE: this fallback is safe for pages without SVG: it will just return the cleaned .equ content.
  // IDs/structure of the exercises are not assumed.
  // Inspired by "exercices.html" behaviour.
  // ------------------
  // Helper to strip interactive elements from a cloned node
  function stripInteractive(root){
    if(!root) return root;
    const $$ = (sel)=>Array.from(root.querySelectorAll(sel));
    // inputs/textarea -> underline
    $$('.tick, .optionline, .kbd-host, [data-math-kbd], button, select, option').forEach(n=>n.remove());
    $$('.frac').forEach(n=>n); // keep
    $$('.input, input, textarea').forEach(el=>{
      // compute width fallback
      const sw = parseInt((el.style && el.style.width || '').replace('px',''))||0;
      const sz = el.getAttribute && el.getAttribute('size') ? parseInt(el.getAttribute('size'),10)*8 : 0;
      const w  = Math.max(40, sw || sz || 80);
      const span = document.createElement('span');
      span.setAttribute('style',`display:inline-block;border-bottom:1px solid #000;min-width:${w}px;height:1.2em;vertical-align:baseline`);
      el.replaceWith(span);
    });
    $$('.steps .tick').forEach(n=>n.remove());
    $$('.controls').forEach(n=>n.remove());
    $$('.header').forEach(n=>n.remove());
    $$('.kbd-host').forEach(n=>n.remove());
    $$('.hide-print').forEach(n=>n.remove());
    $$('.equ [contenteditable], [contenteditable]').forEach(n=>n.removeAttribute('contenteditable'));
    return root;
  }

  // Export curve to HTML (prefers global cloneCurveForPDF/cloneCurveForPDF_img when available)
  function exportCurveHTML(root){
    try{
      if (typeof window.cloneCurveForPDF_img === 'function')
        return window.cloneCurveForPDF_img(root, {widthPx:360, blockWidth:'100%'});
      if (typeof window.cloneCurveForPDF === 'function')
        return window.cloneCurveForPDF(root, {widthPx:360, blockWidth:'100%'});
      // fallback to internal helper at file end
      return (typeof cloneCurveForPDF === 'function') ? cloneCurveForPDF(root, {widthPx:360, blockWidth:'100%'}) : '';
    }catch(e){ return ''; }
  }

  // ---- Actual fallback ----
  // 1) If def.textHTML/text exists, prefer it
  if (typeof def.textHTML === 'function') return def.textHTML(st);
  if (typeof def.text === 'function')     return def.text(st);

  // 2) Offscreen container with NON-ZERO size (critical for SVG text layout)
  if (typeof def.render === 'function') {
    try {
      const tmp = document.createElement('div');
      Object.assign(tmp.style, {
        position:'fixed', left:'-10000px', top:'-10000px',
        width:'420px', height:'420px', overflow:'hidden', pointerEvents:'none'
      });
      document.body.appendChild(tmp);

      def.render(tmp, st);

      // wipe user layer if present (clicks etc.)
      const userLayer = tmp.querySelector('g[data-layer="user"]');
      if (userLayer) userLayer.innerHTML = '';

      // Right = cleaned statement (equ clone preferred)
      const equ = tmp.querySelector('.equ') || tmp;
      const right = equ.cloneNode(true);
      stripInteractive(right);

      // Left = curve if any
      const hasSVG = !!tmp.querySelector('svg');
      const leftHTML = hasSVG ? exportCurveHTML(tmp) : '';

      // 2-col if SVG present, else stacked
      const statement = (hasSVG && right.innerHTML.trim())
        ? `<table style="width:100%;border-collapse:collapse;table-layout:fixed">
             <tr>
               <td style="width:54%;vertical-align:top;padding:0 12px 0 0">${leftHTML}</td>
               <td style="width:46%;vertical-align:top;padding:0">${right.innerHTML}</td>
             </tr>
           </table>`
        : `<div>${leftHTML}</div><div style="margin-top:6px">${right.innerHTML}</div>`;

      tmp.remove();
      return statement && statement.trim() ? statement : 'Énoncé indisponible';
    } catch(e){
...
// Renders offscreen with non-zero size, invokes def.solution/def.correct on the SAME state,
  // strips interactive bits and returns ONLY the solution block HTML.
  // Returns a STRING.
  if (typeof def.render === 'function' && (typeof def.solution === 'function' || typeof def.correct === 'function')) {
    try {
      ...
      // turn ticks & inputs into static
      tmp.querySelectorAll('input, textarea, button, select, option, .tick, .optionline, [data-math-kbd], .kbd-host').forEach(n=>n.remove());
      tmp.querySelectorAll('[contenteditable]').forEach(n=>n.removeAttribute('contenteditable'));

      // Prefer .steps, else .solution, else entire node
      const steps = tmp.querySelector('.steps') || tmp.querySelector('.solution') || tmp;
      const sol = steps.outerHTML || steps.innerHTML || '';
      tmp.remove();
      return sol && sol.trim() ? sol : 'Corrigé indisponible';
    } catch(e) {
      console.warn('[exo-pdf-kit] fallback solution failed', e);
    }
  }
  return 'Corrigé indisponible';
}
  ...
  // 🔽 Anti-doublon pour CETTE génération PDF
  const SEEN = new Set();
  ...
  for (let i=1; i<=nb; i++) {
      ...
      // 1) ÉTAT
      // 1) ÉTAT (tirage avec anti-doublon local)
let st = {};
let tries = 0, sig = '';
do {
